...
export const ArtisanOnboardingModal: React.FC<ArtisanOnboardingModalProps> = ({
  ...
  const [skillsText, setSkillsText] = useState<string>("PPR Welding, Unclogging, Pipe Fitting");
  const [hourlyRate, setHourlyRate] = useState<number>(4000);
  const [hub, setHub] = useState<string>("Unilag Akoka Campus");
  const [bio, setBio] = useState<string>("");
  const [nin, setNin] = useState<string>("12345678901");
  const [bvn, setBvn] = useState<string>("22233344455");
